...
data:
  config.yaml: |
    ...
    static_resources:
            ...
            typed_config:
              ...
              route_config:
                ...
                - name: gateway
                  ...
                  {{- /* Router routes — with cookie affinity */}}
                  {{- if .Values.upstreams.router.enabled }}
                  - match:
                  ...
                  {{- /* Service routes — with rate limits and custom matching */}}
                  {{- if $envoy.serviceRoutes }}
                  {{- toYaml $envoy.serviceRoutes | nindent 18 }}
                  {{- else }}
                  - match:
                      prefix: /api/
                    route:
                  ...
                  {{- /* UI catch-all */}}
                  {{- if .Values.upstreams.ui.enabled }}
                  - match:
                      prefix: /
                    route:
                      cluster: osmo-ui
                ...
                typed_config:
                  ...
                  extension_config:
                    ...
                    typed_config:
                      ...
                      http_service:
                        server_uri:
                          uri: http://{{ include "gateway.fullname" . }}-oauth2-proxy:{{ .Values.oauth2Proxy.httpPort }}/oauth2/auth
                          cluster: oauth2-proxy
                          timeout: 3s
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ .Values.upstreams.service.host }}
                    port_value: {{ .Values.upstreams.service.port }}
      ...
      {{- /* Upstream: osmo-router (ring hash for cookie affinity) */}}
      {{- if .Values.upstreams.router.enabled }}
      - name: osmo-router
        connect_timeout: 3s
        type: STRICT_DNS
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ .Values.upstreams.router.host }}
                    port_value: {{ .Values.upstreams.router.port }}
        {{- if $.Values.tls.enabled }}
        transport_socket:
          name: envoy.transport_sockets.tls
          typed_config:
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ .Values.upstreams.ui.host }}
                    port_value: {{ .Values.upstreams.ui.port }}
        {{- if $.Values.tls.enabled }}
        transport_socket:
          name: envoy.transport_sockets.tls
          typed_config:
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ include "gateway.fullname" . }}-oauth2-proxy
                    port_value: {{ .Values.oauth2Proxy.httpPort }}
      {{- end }}
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ include "gateway.fullname" . }}-authz
                    port_value: {{ .Values.authz.grpcPort }}
      {{- end }}
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ include "gateway.fullname" . }}-ratelimit
                    port_value: {{ .Values.rateLimit.grpcPort }}
      {{- end }}

      {{- /* IDP cluster for JWKS */}}
      {{- if $envoy.idp.host }}
      - name: idp
        connect_timeout: 3s
        type: STRICT_DNS
      ...
      {{- /* Internal JWKS cluster (points to osmo-service for the "osmo" JWT issuer) */}}
        ...
        load_assignment:
          ...
          - lb_endpoints:
            - endpoint:
                address:
                  socket_address:
                    address: {{ $envoy.internalJwks.host | default .Values.upstreams.service.host }}
                    port_value: {{ $envoy.internalJwks.port | default .Values.upstreams.service.port }}
      {{- end }}
