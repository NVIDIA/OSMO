...
DB_SSL_MODE="${PGSSLMODE:-require}"
export PGSSLMODE="$DB_SSL_MODE"

ENCODED_USER=$(urlencode "$DB_USER")
ENCODED_PASSWORD=$(urlencode "$DB_PASSWORD")
ENCODED_SSL_MODE=$(urlencode "$DB_SSL_MODE")
PGROLL_QUERY="sslmode=${ENCODED_SSL_MODE}"
if [ -n "${PGSSLROOTCERT:-}" ]; then
    ENCODED_SSL_ROOT_CERT=$(urlencode "$PGSSLROOTCERT")
    PGROLL_QUERY="${PGROLL_QUERY}&sslrootcert=${ENCODED_SSL_ROOT_CERT}"
fi
export PGROLL_URL="postgres://${ENCODED_USER}:${ENCODED_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}?${PGROLL_QUERY}"
