...
sidecars:
  authz:
    ...
    postgres:
      # Connection settings (host, port, database, user, password) are inherited from services.postgres
...
The authz sidecar inherits connection settings from `services.postgres`:
- `services.postgres.serviceName` - Database host
- `services.postgres.port` - Database port
- `services.postgres.db` - Database name
- `services.postgres.user` - Database user
- `services.postgres.passwordSecretName` - K8s secret name
- `services.postgres.passwordSecretKey` - K8s secret key

Additional connection pool settings in `sidecars.authz.postgres`:
...
sidecars:
  authz:
    postgres:
      ...
      minConns: 2                 # Minimum connections in pool
      maxConnLifetimeMin: 5       # Max lifetime of a connection (minutes)
```
