...
validate_secret() {
    ...
    [ "$managed_by" = osmo-oauth-cookie-bootstrap ] \
        && [ "$instance" = "$release_name" ] || return 1
    ...
    [ "${#decoded_value}" -eq 44 ] || return 1
    case "$decoded_value" in *[!A-Za-z0-9_=-]*) return 1 ;; esac
    raw_length=$(printf '%s' "$decoded_value" | tr '_-' '/+' \
        | base64 -d | wc -c | tr -d ' ') || return 1
    [ "$raw_length" -eq 32 ] || return 1
...
[ -n "$namespace" ] && [ -n "$release_name" ] \
    && [ -n "$secret_name" ] && [ -n "$secret_key" ] || exit 2
...
if [ -n "$existing_name" ]; then
    ...
    log_error "OAuth cookie Secret $secret_name is invalid"
    exit 1
fi
