...
{{/*
Gateway Envoy config data (without ConfigMap wrapper).
Used for both the ConfigMap content and the checksum annotation.
Routes traffic to multiple backend clusters (osmo-service, osmo-agent, osmo-logger)
instead of a single localhost backend.
*/}}
{{- define "osmo.gateway-envoy-config-data" -}}
...
static_resources:
        ...
        typed_config:
          ...
          route_config:
            ...
            - name: gateway
              ...
              {{- range .Values.services.gateway.backendRoutes }}
              - match:
                  safe_regex:
                    regex: {{ .pathRegex | quote }}
                route:
                  cluster: {{ .cluster }}
                  ...
                  {{- end }}
              {{- end }}
              {{- /* Render the default routes from sidecars.envoy.routes, rewriting cluster: service → osmo-service */ -}}
              {{- toYaml $envoy.routes | replace "cluster: service" (printf "cluster: %s" .Values.services.service.serviceName) | nindent 14 }}
                ...
                typed_config:
                  ...
                  providers:
                    ...
                    provider_{{ $i }}:
                      ...
                      remote_jwks:
                        http_uri:
                          {{- if eq $provider.cluster "service" }}
                          uri: http://{{ $.Values.services.service.serviceName }}:8000/api/auth/keys
                          cluster: {{ $.Values.services.service.serviceName }}
                          {{- else }}
  ...
  {{- range .Values.services.gateway.backends }}
  - name: {{ .serviceName }}
    connect_timeout: 3s
    type: STRICT_DNS
    dns_lookup_family: V4_ONLY
