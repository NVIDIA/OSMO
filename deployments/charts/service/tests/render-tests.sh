...
if [[ $(grep -c -- '--secret_name' <<<"$multiple_render") -ne 2 ]]; then
...
upgrade_render=$(helm template upgrade-test "$CHART_DIR" "${helm_args[@]}" \
    ...
    --set 'services.backendApiTokens.credentials[0].name=default' \
    --set 'services.backendApiTokens.credentials[0].managedSecret.name=agent-token')
grep -q -- '--fail_if_missing' <<<"$upgrade_render"
if [[ $(grep -c 'hook-failed' <<<"$upgrade_render") -ne 4 ]]; then
    echo 'Bootstrap hook resources do not all clean up after failure' >&2
    exit 1
fi
