...
verify ConfigMap adoption before the 6.4 DB-config cutover.
...
# it from externalUrl and the adoption gate must catch a wrong public URL.
...
def export_roles(base_url, headers):
    ...
    for role in data:
        ...
        name = role.get('name')
        if name:
            items[name] = strip_fields(role, ROLE_INTERNAL_FIELDS | {'name'})
    return items
...
def load_secret_mappings(path):
    ...
    for index, entry in enumerate(entries):
        ...
        if len(secret_name) > 253 or not SECRET_NAME_PATTERN.fullmatch(secret_name):
            raise ValueError(
        ...
        reference = {'secretName': secret_name}
        secret_key = entry.get('secretKey')
        if secret_key is not None:
            if not isinstance(secret_key, str) or not secret_key:
                raise ValueError(
                    f'Secret mapping {index}.secretKey must be non-empty.')
            ...
            reference['secretKey'] = secret_key
...
def validate_secret_references(value, path=(), secrets_root=None):
    ...
    if 'secret_file' in reference_keys:
        raise ValueError(
            ...
            'ConfigMap adoption; use secretName and optional secretKey.')
    if 'secretKey' in reference_keys and 'secretName' not in reference_keys:
        raise ValueError(
            f'{_format_mapping_path(path)}.secretKey requires secretName.')
    if 'secretName' in reference_keys:
        secret_name = value['secretName']
        _validate_secret_name(secret_name, path)
        if 'secretKey' in reference_keys:
        ...
        if secrets_root:
            ...
            resolved_path = os.path.abspath(os.path.join(
                secret_directory, secret_key or 'cred.yaml'))
...
def _walk_unresolved_secret_references(value, path=()):
    ...
    if isinstance(value, list):
        ...
        if SECRET_REFERENCE_KEYS.intersection(value):
            paths.append(path)
        for key, child in value.items():
            paths.extend(
...
def load_rendered_config(path):
    ...
    for document in documents:
        ...
        if isinstance(data, dict) and isinstance(data.get('config.yaml'), str):
            candidates.append(yaml.safe_load(data['config.yaml']))
    ...
    if isinstance(roles, dict):
        for role_config in roles.values():
            if isinstance(role_config, dict):
                for field in ROLE_INTERNAL_FIELDS:
                    role_config.pop(field, None)
    # Helm intentionally omits empty managed sections. Treat an omitted empty
    # mapping as equivalent to the API's explicit empty mapping.
    return {
...
def verify_rendered_adoption(
    ...
    # Import lazily so plain export keeps its small dependency footprint. The
    # verifier deliberately reuses the production resolver and Pydantic models.
    ...
    try:
        ...
        errors = configmap_loader._validate_configs(resolved)  # pylint: disable=protected-access
    finally:
...
def collect_configs(base_url, headers):
    """Fetch all nine managed sections; a partial export is never safe."""
    configs = {}
...
def build_helm_values(configs, chart):
...
def main():
    parser = argparse.ArgumentParser(
        description='Export and verify OSMO ConfigMap adoption.',
    ...
    try:
        ...
        if args.verify_rendered:
            verify_rendered_adoption(
                configs, args.verify_rendered, args.secrets_root,
                required_secret_paths)
            print('ConfigMap adoption verification passed.', file=sys.stderr)
    except (OSError, ValueError, yaml.YAMLError) as error:
        print(f'Error: {error}', file=sys.stderr)
        return 2

    output = build_helm_values(configs, args.chart)
