...
| Database migration | The pgroll ConfigMap and Job are renamed and ordered ahead of service-auth. The legacy render bundles nine migrations; the proposed chart bundles only the five OSMO 6.4 migrations needed on the guaranteed 6.3 baseline. Password delivery changes from Vault to a typed Secret. | **Resolved.** The per-file runner skips migrations recorded as complete, propagates all other pgroll and psql failures, and honors the chart PostgreSQL TLS mode. Staging owns the `-26` Argo wave; migration success remains a cutover gate. |
