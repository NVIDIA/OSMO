...
// Function to update all links and text containing the embedded domain
function updateDomainReferences() {
    const currentDomain = getCurrentDomain();
    const oldDomain = window.DomainUpdaterConfig.oldDomain;
    ...
    while (node = walker.nextNode()) {
        if (node.textContent.includes(oldDomain)) {
    ...
    document.querySelectorAll('a').forEach(link => {
        if (link.href.includes(oldDomain)) {
            link.href = link.href.replace(oldDomain, currentDomain);
        }
    });
}
