...
``node_conditions`` lets you customize how OSMO evaluates `Kubernetes Node conditions <https://kubernetes.io/docs/reference/node/node-status/#condition>`_ when deciding whether a node is available for scheduling. It also controls the label prefix used for the automatic verification label.

- ``rules``: A mapping from condition type regex to allowed status regex. Status values are the literals ``True``, ``False``, ``Unknown`` and should be combined with ``|`` (OR). Examples: ``{"^Ready$": "True"}``, ``{"^MemoryPressure$": "False|Unknown"}``, ``{"^nvidia.com/.*": "True"}``.
...
- OSMO reads all conditions on a node. A condition is evaluated against all ``rules`` whose regex matches the condition type. If at least one matching rule allows the condition's status (full match against the status regex), that condition passes. If one or more rules match but none allows the status, the node is considered unavailable.
- Conditions that do not match any rule are ignored.
- ``Ready`` is always required to be ``True`` by default; attempts to override it to any other value are rejected.
...
.. note::
  ...
  - Regex match is controlled by your pattern. Use ``^`` and ``$`` for exact matches; use ``.*`` for prefixes (e.g., ``^nvidia.com/.*``).
  - ``Ready`` must remain ``True``. Attempts to set other values are rejected by the backend listener.
