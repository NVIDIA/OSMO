...
* Access to the following container registries:
   ...
   * ``nvcr.io/nvstaging/osmo/``
   ...
   .. TODO: Update this registry to point to the public registry when we switch to GitHub.
...
2. Post-Installation Keycloak Configuration:
   ...
   d. Configure client settings for OSMO application:
   ...
   e. Note down the client secret from the Credentials tab

3. Configure the auth URL in your OSMO values file:

   .. code-block:: yaml

      auth:
        enabled: true
        device_endpoint: https://auth-<example.com>/realms/osmo/protocol/openid-connect/auth/device
        device_client_id: osmo-device
        browser_endpoint: https://auth-<example.com>/realms/osmo/protocol/openid-connect/auth
        browser_client_id: osmo-browser-flow
        token_endpoint: https://auth-<example.com>/realms/osmo/protocol/openid-connect/token
        logout_endpoint: https://auth-<example.com>/realms/osmo/protocol/openid-connect/logout
...
.. important::
   ...
   This is not recommended for production environments. When auth is disabled, all users will be
   able to access all OSMO APIs.

1. Configure the auth URL in your OSMO values file:
...
Create ``osmo_values.yaml`` for osmo with the following sample configurations:
  ...
  .. code-block:: yaml
    ...
    sidecars:
      # Service-specific Envoy overrides
      envoyOverrides:
        service:
