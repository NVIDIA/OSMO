...
OSMO supports periodic data checkpointing from your workflow to a data store.

The ``checkpoint`` field is a list of parameters for each checkpoint process.
For each checkpoint process, you must specify the following parameters:
...
..  list-table::
    ...
    * - checkpoint_cadence (required)
      - How long to wait between one checkpoint ending and the next one beginning.
...
Workflow logs will report the checkpointing process as it runs.
