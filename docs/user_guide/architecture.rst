...
**Scheduler**
  Leverages `NVIDIA Run:AI <https://developer.nvidia.com/blog/nvidia-open-sources-runai-scheduler-to-foster-community-collaboration/>`_
...
1. :bdg-primary:`User` submits a workflow via the CLI or Web UI
2. :bdg-primary:`API Server` authenticates the user and validates the workflow specification
3. :bdg-primary:`Workflow Engine` parses and builds the workflow execution graph (`DAG <https://en.wikipedia.org/wiki/Directed_acyclic_graph>`_ of tasks)
4. :bdg-primary:`Workflow Engine` enqueues tasks to the :bdg-primary:`Scheduler` for resource allocation
...
After a task is enqueued to the :bdg-primary:`Scheduler`:

1. :bdg-primary:`Scheduler` evaluates the task's resource requirements
2. :bdg-primary:`Scheduler` assigns the task to appropriate pools and platforms with available capacity
3. :bdg-primary:`Compute Cluster` creates a pod with specified container image, credentials, and environment variables
...
:bdg-primary:`Workflow Engine` monitors task states and orchestrates execution based on
...
1. :bdg-completed:`Completed` tasks trigger their dependent downstream tasks
2. :bdg-failed:`Failed` retryable tasks are re-enqueued for **automatic recovery**
3. :bdg-failed:`Failed` non-retryable tasks block their downstream dependencies
...
Tasks publishes logs and metrics to the :bdg-primary:`API Server`, enabling **real-time observability**.
...
:bdg-primary:`User` can access :bdg-running:`RUNNING` tasks for real-time interactive development
and debugging:
...
1. :bdg-primary:`User` initiates interactive access through the :bdg-primary:`API Server`
2. :bdg-primary:`API Server` coordinates with the :bdg-primary:`Compute Nodes` to establish a secured connection
3. :bdg-primary:`User` can interact with the :bdg-primary:`Compute Nodes` using the bi-directional streaming connection
...
.. seealso::
  ...
  For detailed deployment procedures, refer to the :doc:`Deployment Guide </deployment_guide/index>`.
