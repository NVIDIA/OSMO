...
![image](deployment_guide/appendix/deploy_minimal.svg)

## Prerequisites

Refer to [Cloud](../requirements/prereqs.md#prerequisites) for the setup of the Kubernetes cluster, PostgreSQL database, and Redis instance.
...
4. Port forward to access the OSMO UI:
   ...
   Visit [http://localhost:3000](http://localhost:3000) in your web browser to access the OSMO UI dashboard as a guest user.
...
1. **Access OSMO Service API**:
   ...
   Then access the OSMO API at [http://localhost:9000/api/docs](http://localhost:9000/api/docs) in your web browser. You can interact with the API using the OSMO CLI.
   ```bash
   $ osmo login http://localhost:9000 --method=dev --username=testuser
...
2. **Access OSMO UI** (in a separate terminal):
   ...
   Then access the OSMO UI at [http://localhost:3000](http://localhost:3000) in your web browser. You should be able to see the OSMO UI dashboard as a guest user.
...
1. Follow the [Configure Data Storage](../getting_started/configure_data.md#configure-data) guide to setup data storage.
2. Follow the [Install Dependencies](../install_backend/dependencies/dependencies.md#installing-required-dependencies) guide to install the KAI scheduler for running workflows.
...
Follow the [Run Workflows](../install_backend/validate_osmo.md#validate-osmo) guide to test basic OSMO functionality.
...
1. Consider upgrading to production deployment ([Deploy Service](../getting_started/deploy_service.md#deploy-service))
2. Setup SSO authentication and authorization ([Advanced Keycloak Configuration](authentication/authentication_flow.md#authentication-flow-sso))
3. Configure persistent storage ([Configure Data Storage](../getting_started/configure_data.md#configure-data))
4. Add observability and monitoring solutions ([Add Observability (Optional)](../install_backend/observability.md#adding-observability))
