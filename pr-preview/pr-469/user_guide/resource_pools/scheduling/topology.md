...
|-------------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `key`             | *(required)*  | The topology level to constrain. Must match one of the keys configured in the pool<br/>(e.g., `rack`, `zone`, `gpu-clique`). Keys represent levels of physical hierarchy<br/>in the cluster, ordered from coarsest to finest<br/>(e.g., `zone` → `spine` → `rack` → `gpu-clique`).            |
| `group`           | `default`     | A name that groups tasks which must share the same physical location at the given<br/>topology level. Tasks sharing the same `key` and `group` will be co-located.<br/>Tasks with different `group` values for the same `key` may land on separate<br/>physical units (e.g., separate racks). |
| `requirementType` | `required`    | Controls whether the constraint is hard or soft. `required` means the workflow will<br/>not be scheduled unless the constraint can be satisfied. `preferred` means OSMO will<br/>attempt to satisfy the constraint but will still schedule the workflow if it cannot.                         |
...
![Topology constraint tree showing zone and gpu-clique levels with task leaf nodes](user_guide/resource_pools/scheduling/topology_tree.svg)

#### SEE ALSO
To add topology requirements to your workflow spec, see [Resources](../../workflows/specification/resources.md#workflow-spec-resources).
...
![Topology constraint tree for single NVL72 rack use case](user_guide/resource_pools/scheduling/topology_tree_uc1.svg)
...
![Topology constraint tree for multiple NVL72 racks use case](user_guide/resource_pools/scheduling/topology_tree_uc2.svg)
...
![Topology constraint tree for multiple NVL72 racks in same zone use case](user_guide/resource_pools/scheduling/topology_tree_uc3.svg)
...
![Topology constraint tree for best-effort topology use case](user_guide/resource_pools/scheduling/topology_tree_uc4.svg)
...
![Topology constraint tree showing that rack-group-1 resolves independently under each zone](user_guide/resource_pools/scheduling/topology_tree_uc5.svg)
