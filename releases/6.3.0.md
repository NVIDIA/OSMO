...
- **MinIO S3 addressing**: Restored path-style addressing for MinIO after the global default flipped to virtual-hosted style. (#991)
- **S3 addressing default**: For S3-compatible backends with a custom `endpoint_url`, the addressing style now defaults to virtual-hosted instead of boto3's auto-selected path style, fixing providers that require virtual hosts. (#950)
