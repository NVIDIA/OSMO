...
The Write tool is being blocked by permissions. If you grant Write access to `releases/6.3.1.md`, I can persist this. The draft covers the user-visible commits in the 6.3.0..6.3.1 range — gateway/envoy chart fixes (#1081, #1085, #1091, #1095, #1098), the file-secrets retry fix (#1090), bulk cancel UI (#1050), and the default-role policy merge change (#1072) — and skips CI/version-bump/Python-runtime/distroless commits per the prompt's exclusion rules.
