...
PACKAGE_HOST_URL="https://github.com/NVIDIA/osmo/releases/latest/download"
...
if [[ "$OS_TYPE" == "Darwin" ]]; then
    PACKAGE_NAME="osmo-client-macos-installer.pkg"
    ...
    if [[ "$CPU_ARCH" == "x86_64" ]]; then
        PACKAGE_NAME="osmo-client-linux-installer-x86_64.sh"
    elif [[ "$CPU_ARCH" == "arm64" ]] || [[ "$CPU_ARCH" == "aarch64" ]]; then
        PACKAGE_NAME="osmo-client-linux-installer-arm64.sh"
    fi
