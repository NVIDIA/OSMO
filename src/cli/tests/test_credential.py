...
class TestSetupParser(unittest.TestCase):
    ...
    def test_set_command_payload_help_avoids_ascii_table(self):
        """Test that payload help stays RST-friendly for autogenerated docs."""
        parser = self._build_parser()
        help_text = parser.format_help()
        self.assertIn('Payload keys by credential type:', help_text)
        self.assertNotIn('+-----------------', help_text)
        self.assertIn('addressing_style', help_text)
