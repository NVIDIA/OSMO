...
"""
Smoke test for Google Cloud Storage.
"""
...
class GoogleCloudStorageTest(smoke.SmokeTestBase):
    """
    Smoke test for Google Cloud Storage that extends the base test class.
    """

    storage_uri = 'gs://osmo-test-bucket/client-library/tests'
    ...
    def test_endpoint_remote_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOCredentialError):
            storage.Client.create(
                storage_uri='gs://expected-bucket/client-library/tests',
                data_credential=credentials.DataCredential(
                    endpoint='gs://incorrect-bucket',
                    access_key_id='test',
    ...
    def test_remote_path_storage_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOUsageError):
            self.test_storage_client.download_objects(
                self.temp_dir,
                source='gs://osmo-test-bucket/incorrect-dir/file.txt',
            )
