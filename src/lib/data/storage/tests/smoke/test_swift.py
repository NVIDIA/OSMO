...
class SwiftStorageTest(smoke.SmokeTestBase):
    """
    Smoke test for Swift storage that extends the base test class.
    """

    storage_uri = 'swift://pdx.s8k.io/AUTH_team-osmo-ops/dev/client-library/tests'
    ...
    def test_endpoint_remote_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOCredentialError):
            storage.Client.create(
                storage_uri='swift://expected-host/namespace/container',
                data_credential=credentials.DataCredential(
                    endpoint='swift://expected-host/incorrect-namespace/container',
                    access_key_id='test',
    ...
    def test_remote_path_storage_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOUsageError):
            self.test_storage_client.download_objects(
                self.temp_dir,
                source='swift://pdx.s8k.io/AUTH_team-osmo-ops/incorrect-container/file.txt',
            )
