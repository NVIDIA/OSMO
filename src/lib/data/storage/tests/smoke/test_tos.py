...
"""
Smoke test for Bytedance Torch Object Storage.
"""
...
class TorchObjectStorageTest(smoke.SmokeTestBase):
    """
    Smoke test for Bytedance Torch Object Storage that extends the base test class.
    """

    storage_uri = 'tos://tos-s3-cn-shanghai.volces.com/nv-osmo-bucket/client-library/tests'
    ...
    def test_endpoint_remote_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOCredentialError):
            storage.Client.create(
                storage_uri='tos://expected-host/container',
                data_credential=credentials.DataCredential(
                    endpoint='tos://expected-host/incorrect-container',
                    access_key_id='test',
    ...
    def test_remote_path_storage_uri_mismatch(self):
        ...
        with self.assertRaises(osmo_errors.OSMOUsageError):
            self.test_storage_client.download_objects(
                self.temp_dir,
                source='tos://tos-s3-cn-shanghai.volces.com/incorrect-container/file.txt',
            )
