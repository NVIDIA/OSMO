...
func (nl *NodeListener) sendMessages(
	...
	for {
		...
		case msg, ok := <-ch:
			if !ok {
				nl.inst.MessageChannelClosedUnexpectedly.Add(ctx, 1, nl.Attrs)
...
func (nl *NodeListener) watchNodes(
	...
	handleNodeEvent := func(node *corev1.Node, isDelete bool) {
		nl.inst.KubeEventWatchCount.Add(ctx, 1, nl.Attrs)

		msg := nl.buildResourceMessage(node, nodeStateTracker, isDelete, labelUpdateChan)
		if msg != nil {
			select {
			case nodeChan <- msg:
				nl.inst.MessageQueuedTotal.Add(ctx, 1, nl.Attrs)
				nl.inst.MessageChannelPending.Record(ctx, float64(len(nodeChan)), nl.Attrs)
			case <-done:
				return
			}
		}
	...
	nodeInformer.SetWatchErrorHandler(func(r *cache.Reflector, err error) {
		...
		nl.inst.EventWatchConnectionErrorCount.Add(ctx, 1, nl.Attrs)
...
func (nl *NodeListener) rebuildNodesFromStore(
	...
	nl.inst.InformerRebuildTotal.Add(ctx, 1, nl.Attrs)
