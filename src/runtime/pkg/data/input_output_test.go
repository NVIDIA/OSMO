...
// ---------------------------------------------------------------------------
// Helpers for Download / UploadFolder tests
// ---------------------------------------------------------------------------
...
// fakeBinaries installs fake `osmo` and `tree` binaries in a temp directory and
// points PATH / TREE_PATH at them. Each script body is the body of the script
// (the shebang is added automatically). Use empty strings for default no-op
// behavior.
func fakeBinaries(t *testing.T, osmoBody, treeBody string) {
	t.Helper()
	binDir := t.TempDir()
	if osmoBody == "" {
		osmoBody = "exit 0"
...
// captureResult is what runWithCapture collects.
...
// runWithCapture collects everything written to osmoChan / metricChan while fn
...
// ---------------------------------------------------------------------------
// TaskInput.Download — exec-based happy path
// ---------------------------------------------------------------------------

func TestTaskInput_Download_LogsDownloadedAndCreatesFolder(t *testing.T) {
	...
	res := runWithCapture(func(osmoChan chan string, metricChan chan metrics.Metric) {
		ti.Download(conn, inputPath, osmoChan, metricChan, "rid", "g", "tn", 0)
	})
