...
def select_targets(
    ...
    Args:
        ...
        max_lines_per_target: Cap on uncovered lines per target (0 = no cap).
    ...
    for file_report in report.get("files", []):
        ...
        covered = sum(1 for _, status in line_coverage if status == 0)
        coverage_pct = (covered / total_lines * 100) if total_lines else 0.0

        uncovered_numbers = sorted(
            line for line, status in line_coverage if status == 1
        )
        uncovered_ranges = _lines_to_ranges(uncovered_numbers)

        if max_lines_per_target > 0:
            uncovered_ranges = _cap_ranges(uncovered_ranges, max_lines_per_target)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Max uncovered lines per target (default: 200, 0 = no cap)",
    )
    args = parser.parse_args()

    token = args.token or __import__("os").environ.get("CODECOV_TOKEN", "")
    if not token:
    ...
    targets = select_targets(report, args.max_targets, args.max_lines)
