...
"""Create a PR from test files generated by the agent.
...
# Match SUSPECTED BUG markers the agent leaves on skipped tests. Accepts
...
# The second form is what surfaced on PR #1046 — the agent wrote the marker
...
def _scan_suspected_bugs(files: list[str]) -> list[str]:
    """Scan test files for SUSPECTED BUG markers left by the agent."""
    seen: set[str] = set()
    bugs: list[str] = []
...
Generated by testbot pipeline"""
    ...
    if pr_url and (output := os.environ.get("GITHUB_OUTPUT")):
        with open(output, "a", encoding="utf-8") as stream:
            stream.write(f"pr_url={pr_url}\n")
