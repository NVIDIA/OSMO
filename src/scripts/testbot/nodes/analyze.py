...
from testbot.lcov_parser import CoverageEntry, parse_lcov
...
def analyze_coverage(state: TestbotState) -> TestbotState:
    """LangGraph node: fetch coverage data and populate targets."""
    coverage_source = state.get("coverage_source", "codecov")

    if coverage_source == "codecov":
        all_entries = _fetch_from_codecov(state)
    else:
        all_entries = _fetch_from_local_lcov(state)
...
def _fetch_from_codecov(state: TestbotState) -> list[CoverageEntry]:
    """Fetch coverage from Codecov API (fast, uses data from self-hosted CI)."""
    token = state.get("codecov_token", "")
    if not token:
        logger.warning("No Codecov token provided, falling back to local LCOV files")
        return _fetch_from_local_lcov(state)

    return fetch_coverage(token=token)


def _fetch_from_local_lcov(state: TestbotState) -> list[CoverageEntry]:
    """Parse coverage from local LCOV files (requires bazel coverage to have run)."""
    all_entries = []
