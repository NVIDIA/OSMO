...
class LLMClient(LLMProvider):
    ...
    def __init__(self, provider: str = "nemotron"):
        ...
        if OpenAI is None:
            logger.warning("openai package not installed. LLMClient will not work.")
            self.client = None
            return

        if not api_key:
            logger.warning("No API key for provider=%s (set NVIDIA_API_KEY)", provider)
