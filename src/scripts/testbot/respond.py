...
def _find_unresponded_comments(pr_number: int) -> list[dict]:
    ...
    for comment in all_comments:
        ...
        if skip_reason:
            logger.debug("Skip comment %d: %s", comment["id"], skip_reason)
            continue
        unresponded.append(comment)

    logger.info("Found %d unresponded comments", len(unresponded))
    return unresponded
...
def _process_file_comments(
    ...
    user_prompt = build_batch_respond_prompt(file_path, file_content, comment_dicts)
