...
# Authors whose comments should never trigger a response (prevents loops)
...
def should_skip_comment(
    ...
    # Never respond to own comments (prevents infinite loops)
    if author in SELF_AUTHORS:
        return None  # silent skip — no reply to avoid more loops

    # Skip bot auto-replies (e.g., CodeRabbit's "Skipped: comment is from another bot")
    if in_reply_to_id is not None and author.endswith("[bot]"):
        return "this is a reply to another bot comment."

    # Skip explicit opt-out
    if body.startswith("[skip-agent]"):
        return None  # silent skip — user explicitly opted out

    # Skip comments on testbot source code
    file_path = comment_data.get("path", "")
    if file_path.startswith("src/scripts/testbot/"):
        return "comment on testbot source code (not a generated test file)."

    # Skip if max responses reached
    if current_response_count >= MAX_AUTO_RESPONSES:
        return f"max auto-responses ({MAX_AUTO_RESPONSES}) reached for this PR."
...
def _parse_llm_response(response: str) -> tuple[str, Optional[str]]:
    """Parse LLM response into (reply_text, fix_content_or_none)."""
    stripped = response.strip()

    if stripped.startswith("FIX:"):
        code_match = re.search(r"```\w*\n(.*?)```", stripped, re.DOTALL)
        reply_match = re.search(r"REPLY:\s*(.*)", stripped, re.DOTALL)
        fix_content = code_match.group(1).strip() + "\n" if code_match else None
        reply_text = reply_match.group(1).strip() if reply_match else "Applied fix."
        return reply_text, fix_content

    if stripped.startswith("REPLY:"):
        return stripped[len("REPLY:"):].strip(), None
...
def respond_to_comment(
    ...
    # Skip checks — silent skip for own comments, reply with reason for others
    if author in SELF_AUTHORS:
        logger.info("Skipping own comment from %s", author)
        return
    ...
    if skip_reason:
        logger.info("Skipping comment %d: %s", comment_id, skip_reason)
        _reply_to_comment(pr_number, comment_id, f"> Skipped: {skip_reason}")
        return
    ...
    # Call LLM with the respond prompt
    ...
    logger.info("LLM response (%d chars): %s", len(response_text), response_text[:200])

    # Parse response
    reply_text, fix_content = _parse_llm_response(response_text)

    if fix_content:
        logger.info("LLM suggested a code fix (%d chars)", len(fix_content))
        # Write fix and validate
        write_file(comment.file_path, fix_content)

        validation = run_test(comment.file_path)
        if not validation.passed:
            logger.warning("Fix validation failed: %s", validation.output[:200])
            _reply_to_comment(
                pr_number, comment_id,
                f"{reply_text}\n\n> Note: tests failed after applying this fix, needs human review.",
            )
            # Restore original file
            write_file(comment.file_path, file_content)
            return
