...
def _reply_to_comment(pr_number: int, comment_id: int, message: str) -> None:
    ...
    repo = _get_repo_nwo()
    run_shell_with_file(
        f"gh api repos/{repo}/pulls/{pr_number}/comments/{comment_id}/replies -F body=@{{}}", message,
    )
...
def _process_file_threads(
    ...
    for attempt in range(1, MAX_FIX_RETRIES + 2):  # +1 for initial attempt, +1 for reply-only fallback
        ...
        if validation.passed:
            ...
            run_shell_with_file("git commit -F {}", commit_msg)
            run_shell("git push")
...
def main():
    """CLI entry point: parse arguments and respond to PR review comments."""
