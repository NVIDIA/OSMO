...
class TestPrCreationHelpers(unittest.TestCase):
    ...
    def test_main_enables_auto_merge_after_pr_create(self):
        ...
        with tempfile.TemporaryDirectory() as summaries, \
                tempfile.NamedTemporaryFile() as step_output, \
                ...
                patch.object(sys, "argv", ["create_pr.py", "--generate-summary",
                      str(Path(summaries) / "generator.md"), "--review-summary",
                      str(Path(summaries) / "review.md")]):
            ...
            (Path(summaries) / "generator.md").write_text(
                "Coverage: 93 / 97 listed lines hit.\n", encoding="utf-8")
            (Path(summaries) / "review.md").write_text(
                "Fixed source bug and enabled regression.\n", encoding="utf-8")
            main()
            command = run_mock.call_args.args[0]
            body = command[command.index("--body") + 1]
            self.assertIn("## Generator summary", body)
            self.assertIn("Coverage: 93 / 97 listed lines hit.", body)
            self.assertIn("## Independent review", body)
            self.assertIn("Fixed source bug and enabled regression.", body)
            self.assertNotIn("## Coverage gain on listed uncovered ranges", body)
