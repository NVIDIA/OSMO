...
import unittest
from testbot.plugins import _instances, get_llm, register_plugin
...
class TestLLMProviderABC(unittest.TestCase):
    """Tests that LLMProvider enforces its abstract interface contract."""
...
class _DummyWriter(LLMProvider):
...
class TestPluginRegistry(unittest.TestCase):
    """Would have caught: get_llm() creating new instances on every call,
    ...
    def test_get_llm_returns_same_instance(self):
        writer1 = get_llm("dummy")
        writer2 = get_llm("dummy")
        self.assertIs(writer1, writer2)

    def test_get_llm_preserves_state_across_calls(self):
        writer = get_llm("dummy")
        writer.generate_test("foo.py", [])
        self.assertEqual(writer.call_count, 1)

        same_writer = get_llm("dummy")
