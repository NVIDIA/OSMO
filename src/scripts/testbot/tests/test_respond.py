...
class TestExtractReplies(unittest.TestCase):
    ...
    def test_tier1_structured_output(self):
        claude_output = {
            "structured_output": {
                "replies": [
                    {"thread_id": "123", "reply": "Added edge case tests."},
                    {"thread_id": "456", "reply": "Fixed the assertion."},
                ],
            },
        }
    ...
    def test_tier2_json_in_result_text(self):
        data = json.dumps({
            "replies": [{"thread_id": "789", "reply": "Done."}],
        })
    ...
    def test_skips_entries_without_thread_id(self):
        claude_output = {
            "structured_output": {
                "replies": [{"reply": "no thread id"}],
            },
        }
        self.assertEqual(_extract_replies(claude_output), {})
