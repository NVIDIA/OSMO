...
from testbot.nodes.write import _apply_build_entry, _generate_default_build_entry
from testbot.tools.test_runner import _run_bazel_test
...
class TestValidationFalsePositive(unittest.TestCase):
    ...
    def test_fails_when_no_bazel_target(self):
        with patch("testbot.tools.test_runner._file_path_to_bazel_target", return_value=None):
    ...
    def test_proceeds_when_target_exists(self):
        with patch("testbot.tools.test_runner._file_path_to_bazel_target",
                    return_value="//src/cli/tests:test_access_token"):
            with patch("testbot.tools.test_runner.run_shell") as mock_shell:
