...
async function expandFiltersIfCollapsed(page: Parameters<typeof setupDefaultMocks>[0]) {
  ...
  if ((await filterToggle.getAttribute("aria-expanded")) === "false") {
    ...
    await expect(filterToggle).toHaveAttribute("aria-expanded", "true", { timeout: 3000 });
  }
}
...
test.describe("Resources List", () => {
  ...
  test("searches resources by hostname", async ({ page }) => {
    await setupResources(page, createResourcesResponse([
      {
  ...
  test("filters by pool", async ({ page }) => {
    await setupPools(page, createPoolResponse([
...
test.describe("Resource Details", () => {
  ...
  test("shows pool memberships for shared resources", async ({ page }) => {
    await setupResources(page, createResourcesResponse([
      {
...
test.describe("Edge Cases", () => {
  ...
  test("handles empty resources gracefully", async ({ page }) => {
    await setupPools(page, createPoolResponse([{ name: "empty", status: PoolStatus.ONLINE }]));
  ...
  test("handles resources with issues", async ({ page }) => {
    await setupResources(page, createResourcesResponse([
      {
  ...
  test("handles CPU-only nodes (no GPU)", async ({ page }) => {
    await setupResources(page, createResourcesResponse([
      {
