...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      PLAYWRIGHT_E2E: "1",
      // Server-side fetches use `getServerApiBaseUrl()`.
      // In dev + mock mode they target localhost:9999 where `e2e/global-setup.ts` starts
      // `mock-api-backend.mjs`. CI does not run `dev:mock`; set this here so E2E SSR fetches
      // are routed to the test stub instead of a real backend.
