...
describe("date-range-utils", () => {
  describe("returns null for invalid input", () => {
    ...
    it("returns_null_for_invalid_month", () => {
      const result = parseDateRangeValue("2024-13-15");
      expect(result).toBeNull();
    });

    it("returns_null_for_invalid_day", () => {
      const result = parseDateRangeValue("2024-06-32");
      expect(result).toBeNull();
    });

    it("returns_null_for_non_iso_format", () => {
    ...
    it("returns_null_for_datetime_with_seconds", () => {
      // Only YYYY-MM-DDTHH:mm format is supported, not seconds
      const result = parseDateRangeValue("2024-06-15T14:30:00");
      expect(result).toBeNull();
    });

    it("returns_null_for_datetime_with_timezone", () => {
      const result = parseDateRangeValue("2024-06-15T14:30Z");
      expect(result).toBeNull();
    });

    it("returns_null_for_invalid_time", () => {
      // Hour 25 is invalid
      const result = parseDateRangeValue("2024-06-15T25:00");
      expect(result).toBeNull();
    });

    it("returns_null_for_unpadded_components", () => {
      // Single-digit month/day without zero padding is rejected
  ...
  describe("parses single ISO date strings", () => {
    ...
    it("parses_year_end_with_rollover", () => {
    ...
    it("parses_datetime_at_2359_with_day_rollover", () => {
  ...
  describe("parses ISO range strings", () => {
    ...
    it("returns_null_for_malformed_range_separator", () => {
      // Single dot is not a valid separator
      const result = parseDateRangeValue("2024-01-01.2024-01-31");
      expect(result).toBeNull();
    });

    it("returns_null_for_triple_dot_separator", () => {
      const result = parseDateRangeValue("2024-01-01...2024-01-31");
      expect(result).toBeNull();
    });

    it("returns_null_for_range_with_empty_parts", () => {
      expect(parseDateRangeValue("..2024-01-31")).toBeNull();
  ...
  describe("parses preset labels", () => {
    ...
    it("parses_preset_case_insensitive", () => {
      const result = parseDateRangeValue("TODAY");
    ...
    it("returns_null_for_preset_with_whitespace", () => {
      // Leading/trailing whitespace prevents preset matching
      expect(parseDateRangeValue("  today")).toBeNull();
      expect(parseDateRangeValue("today  ")).toBeNull();
    });
  });

  describe("DATE_RANGE_PRESETS at year boundary", () => {
    const yearStartDate = new Date("2024-01-05T12:00:00.000Z");

    beforeEach(() => {
      vi.useFakeTimers();
      vi.setSystemTime(yearStartDate);
    });
    ...
    it("last_7_days_crosses_year_boundary", () => {
      const result = parseDateRangeValue("last 7 days");
  ...
  describe("DATE_RANGE_PRESETS", () => {
    ...
    it("range_presets_return_correct_format", () => {
      const preset7 = DATE_RANGE_PRESETS.find((p) => p.label === "last 7 days");
      const preset30 = DATE_RANGE_PRESETS.find((p) => p.label === "last 30 days");
      const preset90 = DATE_RANGE_PRESETS.find((p) => p.label === "last 90 days");
      const preset365 = DATE_RANGE_PRESETS.find((p) => p.label === "last 365 days");

      expect(preset7!.getValue()).toBe("2024-06-08..2024-06-15");
      expect(preset30!.getValue()).toBe("2024-05-16..2024-06-15");
      expect(preset90!.getValue()).toBe("2024-03-17..2024-06-15");
      expect(preset365!.getValue()).toBe("2023-06-16..2024-06-15");
    });
