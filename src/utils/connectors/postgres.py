...
class ResourceSpec(pydantic.BaseModel):
    ...
    def get_allocatable_tokens(self, default_variables: Dict,
        ...
        storage_num, storage_unit = split_num_units(
            str(final_tokens['USER_STORAGE']) if final_tokens.get('USER_STORAGE') else None)
        store_num_units(storage_num, storage_unit, final_tokens, 'USER_STORAGE')
        ...
        memory_num, memory_unit = split_num_units(
            str(final_tokens['USER_MEMORY']) if final_tokens.get('USER_MEMORY') else None)
        ...
        task_cache_size = task_cache_size if task_cache_size\
            else str(final_tokens['USER_CACHE']) if final_tokens.get('USER_CACHE') else None
