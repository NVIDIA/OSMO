...
class CtrlResourceLimitsTest(unittest.TestCase):
    """The ctrl container's resource limits must cover its requests."""

    def test_raises_sub_gib_ctrl_limits_to_requests(self):
        pod: dict[str, Any] = {
            'spec': {
                'containers': [
                    {
                        ...
                        'resources': {
                            ...
                            'limits': {'memory': '512Mi', 'ephemeral-storage': '100Mi'},
                        },
                    },
                ],
            },
        ...
        result = task.ensure_ctrl_resource_limits(pod)

        self.assertIs(result, pod)
        self.assertEqual(pod['spec']['containers'][0]['resources']['limits'], {
            'memory': '1Gi', 'ephemeral-storage': '1Gi',
        })

    def test_leaves_valid_larger_limits_and_user_container_unchanged(self):
        pod: dict[str, Any] = {
            'spec': {
                'containers': [
                    {
        ...
        task.ensure_ctrl_resource_limits(pod)

        self.assertEqual(pod['spec']['containers'][0]['resources']['limits'], {
            'memory': '2Gi', 'ephemeral-storage': '1Gi',
        })
        self.assertEqual(pod['spec']['containers'][1]['resources']['limits'], {
            'memory': '512Mi', 'ephemeral-storage': '100Mi',
        })

    def test_leaves_invalid_ctrl_quantities_unchanged(self):
        pod: dict[str, Any] = {
            'spec': {
                'containers': [{
                    'name': 'osmo-ctrl',
        ...
        self.assertEqual(pod['spec']['containers'][0]['resources']['limits']['memory'], '512Mi')

    def test_raises_decimal_si_limits_below_one_gib_requests(self):
        pod: dict[str, Any] = {
            'spec': {
                'containers': [{
                    'name': 'osmo-ctrl',
                    'resources': {
                        'requests': {'memory': '1Gi', 'ephemeral-storage': '1Gi'},
                        'limits': {'memory': '1.05G', 'ephemeral-storage': '1000M'},
                    },
                }],
            },
        }
    ...
    def test_leaves_decimal_si_limits_above_one_gib_requests_unchanged(self):
        pod: dict[str, Any] = {
            'spec': {
                'containers': [{
                    'name': 'osmo-ctrl',
                    'resources': {
                        'requests': {'memory': '1Gi', 'ephemeral-storage': '1Gi'},
                        'limits': {'memory': '1.1G', 'ephemeral-storage': '1100M'},
                    },
                }],
            },
        }
        ...
        self.assertEqual(pod['spec']['containers'][0]['resources']['limits'], {
            'memory': '1.1G', 'ephemeral-storage': '1100M',
        })

    def test_leaves_malformed_container_structures_unchanged(self):
        pods: list[dict[str, Any]] = [
            {'spec': {'containers': 'osmo-ctrl'}},
            {'spec': {'containers': ['osmo-ctrl']}},
            {'spec': {'containers': [{'name': 'osmo-ctrl', 'resources': 'invalid'}]}},
        ]

        for pod in pods:
            self.assertIs(task.ensure_ctrl_resource_limits(pod), pod)


class DefaultCtrlPodTemplateTest(unittest.TestCase):
    """Default ctrl limits must remain at least its fixed requests."""

    def test_sub_gib_resources_render_to_one_gib_limits(self):
        pod_template = copy.deepcopy(config_objects.DEFAULT_POD_TEMPLATES['default_ctrl'])

        task.substitute_pod_template_tokens(pod_template, {
            'USER_CPU': 1,
            'USER_MEMORY': '512Mi',
            'USER_MEMORY_Gi': 0.5,
            'USER_STORAGE': '100Mi',
            'USER_STORAGE_Gi': 100 / 1024,
        })

        self.assertEqual(pod_template['spec']['containers'][0]['resources']['limits'], {
            'cpu': '1', 'memory': '1Gi', 'ephemeral-storage': '1Gi',
        })

    def test_above_one_gib_resources_remain_user_derived(self):
        pod_template = copy.deepcopy(config_objects.DEFAULT_POD_TEMPLATES['default_ctrl'])
        ...
        self.assertEqual(pod_template['spec']['containers'][0]['resources']['limits'], {
            'cpu': '1', 'memory': '2Gi', 'ephemeral-storage': '4Gi',
        })
