...
export const metadata: Metadata = {
  title: "Datasets | OSMO",
  description: "Browse and manage datasets across storage buckets.",
};

/**
 * Datasets Page (Streaming SSR with Server Prefetch)
 *
 * Architecture: Hybrid streaming for optimal UX
 * 1. Page shell + skeleton stream immediately (fast TTFB)
 * 2. DatasetsWithData suspends while prefetching on server
...
export default function DatasetsPage({ searchParams }: DatasetsPageProps) {
  ...
  // DatasetsWithData suspends and streams when data is ready
  return (
    <Suspense fallback={<DatasetsPageSkeleton />}>
      <DatasetsWithData searchParams={searchParams} />
    </Suspense>
  );
}
