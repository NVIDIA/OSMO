...
function generateDiamondPattern(
  ...
  groups.push(generateGroup(
    ...
    [],
    ["branch-a", "branch-b"],
    [generateTask("start-0", "start", Status.COMPLETED, workflowUuid, baseUrl, startTime, new Date(startTime.getTime() + 120000))],
    startTime,
  ...
  groups.push(generateGroup(
    ...
    ["start"],
    ["merge"],
    [generateTask("branch-a-0", "branch-a", Status.COMPLETED, workflowUuid, baseUrl, branchAStart, new Date(branchAStart.getTime() + 300000))],
    branchAStart,
...
function generateComplexPattern(
  ...
  groups.push(generateGroup(
    ...
    ["fetch"],
    ["train"],
    [generateTask("validate-0", "validate", Status.COMPLETED, workflowUuid, baseUrl, currentTime, new Date(currentTime.getTime() + 120000))],
    currentTime,
  ...
  groups.push(generateGroup(
    ...
    ["fetch"],
    ["train"],
    [generateTask("preproc-b-0", "preproc-b", Status.COMPLETED, workflowUuid, baseUrl, currentTime, new Date(currentTime.getTime() + 480000))],
    currentTime,
  ...
  for (let i = 0; i < 4; i++) {
    const taskStatus = i === 0 ? Status.COMPLETED 
      : i < 3 ? Status.RUNNING 
      : Status.WAITING;
    trainTasks.push(generateTask(
      `train-shard-${i}`,
      "train",
      taskStatus,
      workflowUuid,
  ...
  for (let level = 0; level < numLevels; level++) {
    ...
    for (let lane = 0; lane < groupsInThisLevel; lane++) {
      ...
      for (let t = 0; t < tasksPerGroup; t++) {
        const taskStatus = status === Status.COMPLETED ? Status.COMPLETED
          : status === Status.RUNNING ? faker.helpers.arrayElement([Status.COMPLETED, Status.RUNNING])
          : Status.WAITING;
      ...
      if (level < numLevels - 1) {
        for (let i = 0; i < Math.min(2, groupsPerLevel); i++) {
          const nextIndex = (level + 1) * groupsPerLevel + ((lane + i) % Math.min(groupsPerLevel, numGroups - (level + 1) * groupsPerLevel));
...
function generateMassiveParallelPattern(
  ...
  groups.push(generateGroup(
    ...
    [generateTask("preprocess-0", "preprocess", Status.COMPLETED, workflowUuid, baseUrl, startTime, new Date(startTime.getTime() + 300000))],
    startTime,
  ...
  const trainTasks: TaskQueryResponse[] = [];
  for (let i = 0; i < 200; i++) {
    const taskStatus = i < 50 ? Status.COMPLETED
      : i < 150 ? Status.RUNNING
      : Status.WAITING;
    ...
    trainTasks.push(generateTask(
      ...
      taskStatus === Status.COMPLETED ? new Date(trainStart.getTime() + faker.number.int({ min: 1800000, max: 7200000 })) : null
    ));
  }
...
function generateMultiRootPattern(
  ...
  roots.forEach((name, i) => {
    groups.push(generateGroup(
      ...
      [],
      ["validate"],
      [generateTask(`${name}-0`, name, Status.COMPLETED, workflowUuid, baseUrl, startTime, new Date(startTime.getTime() + (i + 1) * 60000))],
      startTime,
  ...
  groups.push(generateGroup(
    ...
    roots,
    ["train-a", "train-b"],
    [generateTask("validate-0", "validate", Status.COMPLETED, workflowUuid, baseUrl, validateStart, new Date(validateStart.getTime() + 120000))],
    validateStart,
  ...
  for (let i = 0; i < 6; i++) {
    const taskStatus = i < 2 ? Status.COMPLETED
      : i < 5 ? Status.RUNNING
      : Status.WAITING;
    trainATasks.push(generateTask(
      `train-a-shard-${i}`,
      "train-a",
      taskStatus,
      workflowUuid,
  ...
  for (let i = 0; i < 4; i++) {
    const taskStatus = i < 1 ? Status.COMPLETED
      : i < 3 ? Status.RUNNING
      : Status.WAITING;
    trainBTasks.push(generateTask(
      `train-b-shard-${i}`,
      "train-b",
      taskStatus,
      workflowUuid,
