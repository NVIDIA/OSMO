        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5">
          ...
          {breadcrumb && onBack && (
            <>
              <button
                onClick={onBack}
                className="-ml-1 flex shrink-0 items-center gap-1 rounded-md py-1 pl-1 pr-2 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-zinc-200"
          ...
          {hasSiblings ? (
            <DropdownMenu onOpenChange={handleOpenChange}>
              <DropdownMenuTrigger asChild>
                <button
                  className="flex min-w-0 items-center gap-2 rounded-md py-0.5 pl-1.5 pr-1.5 text-zinc-100 transition-colors hover:bg-zinc-800"
                    ...
                    filteredSiblings.map((task) => (
                      ...
                      >
                        ...
                        <span className={cn(
                          ...
                          task.isCurrent ? "font-medium text-zinc-100" : "text-zinc-300",
                        )}>
                          {task.name}
                          {task.retryId > 0 && (
                            <span className="ml-1.5 text-zinc-500">#{task.retryId}</span>
                          )}
                        </span>
                        {/* Current indicator */}
                        {task.isCurrent && (
          ...
          {subtitle && (
            <>
              <span className="shrink-0 text-zinc-600">·</span>
              <span className="shrink-0 text-sm text-zinc-400">{subtitle}</span>
            </>
          )}
        </div>
