    ...
    <div className="relative">
      ...
      >
        ...
        <input
          ...
          className="min-w-[7.5rem] flex-1 bg-transparent outline-none placeholder:text-zinc-400"
        />
      </div>
      ...
      {showDropdown && suggestions.length > 0 && (
        <div
          ref={dropdownRef}
          className="absolute inset-x-0 top-full z-50 mt-1 max-h-80 overflow-auto overscroll-contain rounded-md border border-zinc-200 bg-white shadow-lg dark:border-zinc-700 dark:bg-zinc-900"
        ...
        >
          ...
          {suggestions.map((item, index) =>
            item.type === "hint" ? (
              <div key={`${item.type}-${index}`} className="px-3 py-2 text-sm italic text-zinc-500 dark:text-zinc-400">
                {item.display}
              </div>
            ) : (
              <button
