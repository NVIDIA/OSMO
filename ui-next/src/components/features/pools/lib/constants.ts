...
const STATUS_DISPLAYS: Record<string, StatusDisplay> = {
  [PoolStatus.ONLINE]: { category: "online", label: "Online", icon: "🟢", sortOrder: 0 },
  [PoolStatus.MAINTENANCE]: { category: "maintenance", label: "Maintenance", icon: "🟡", sortOrder: 1 },
  [PoolStatus.OFFLINE]: { category: "offline", label: "Offline", icon: "🔴", sortOrder: 2 },
};

export function getStatusDisplay(status: string): StatusDisplay {
  return STATUS_DISPLAYS[status] ?? { category: "offline", label: status, icon: "⚪", sortOrder: 99 };
}
