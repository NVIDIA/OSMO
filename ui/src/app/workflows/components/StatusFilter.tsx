...
import { WorkflowStatusValues, type WorkflowStatusType } from "~/models";

export const getMapFromStatusArray = (statusArray: string[]) => {
  return new Map(WorkflowStatusValues.map((value) => [value, statusArray.includes(value.toString())]));
};
...
export const getWorkflowStatusArray = (
  ...
  if (statusFilterType === StatusFilterType.FAILED) {
    return WorkflowStatusValues.filter((status) => status.startsWith("FAILED"));
  }
...
export const StatusFilter = ({
  ...
  statusMap: Map<WorkflowStatusType, boolean>;
  setStatusMap: (map: Map<WorkflowStatusType, boolean>) => void;
  className?: string;
  statusFilterType?: StatusFilterType;
